...
class OneOneMI(nn.Module):
    def __init__(self, code_size, global_size, hidden_size, mode='bilinear'):
        super().__init__()
        mode = mode.lower()
        if mode == 'bilinear':
...
class SeqOneMI(nn.Module):
    def __init__(self, code_size, seq_size, hidden_size, mode='bilinear'):
        super().__init__()
        mode = mode.lower()
        if mode == 'bilinear':
...
class KPredLoss(nn.Module):
    def __init__(self, c_size, z_size, k=4, full_ce=True, auto_is_bidirectional=True, look_both=False):
        super().__init__()
        if k <= 0:
            k = 0
        self.k = k
    ...
    def calc_sampled_cross_entropy_loss(self, c_flat, z_flat, weight):
    ...
    def forward(self, c, z):
        ...
        for i in range(self.k):
            ...
            if self.full_ce:
                ...
                f_loss, t_acc, b_acc = self.calc_sampled_cross_entropy_loss(c_flat, z_flat, self.forward_weights[i])
            ...
            if self.bidirectional:
                ...
                if self.full_ce:
                    ...
                    f_loss, t_acc, b_acc = self.calc_sampled_cross_entropy_loss(c_flat, z_flat,
