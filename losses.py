...
def per_disc_loss(d_fake, d_real, pred_hat, x_hat, loss_type, grad_lambda, label_smoothing, use_mixup, iwass_epsilon, d_last_real, d_real2, d_last_real2, LAMBDA_2):
    if loss_type == 'hinge':
...
def disc_loss(d_fake, d_real, disc, real, fake, loss_type, iwass_epsilon, grad_lambda, label_smoothing, use_mixup,
              d_last_real, d_real2, d_last_real2, LAMBDA_2):
    ...
    loss = per_disc_loss(d_fake, d_real, pred_hat, x_hat, loss_type, grad_lambda, label_smoothing, use_mixup,
                         iwass_epsilon, d_last_real, d_real2, d_last_real2, LAMBDA_2)
    return loss


def G_loss(G, D, fake_latents_in, loss_type, label_smoothing):
    G.zero_grad()
    z = Variable(fake_latents_in)
    g_ = G(z)
    d_fake, _ = D(g_)
    return -d_fake.mean().unsqueeze(0)
